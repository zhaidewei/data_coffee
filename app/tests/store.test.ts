...
import {deleteDraft,execute,insertActivity,load,project,advance,sendOrganizerMail} from '../worker/store';
...
describe('D1事务与公开投影',()=>{
 ...
 it('仅发起人看到邮箱，并可在每场五次限额内选择收件人',async()=>{
  let e=await published();
  for(const id of ['a','b']){await env.DB.prepare('INSERT INTO users(id,email,nickname,public_nickname) VALUES(?,?,?,1)').bind(id,`${id}@example.com`,`成员${id}`).run();e=await execute(env,e.id,{action:'join'},user(id),`join-${id}`);}
  expect(JSON.stringify(await project(env,e,null))).not.toContain('@example.com');
  const ownerView=await project(env,e,user('owner')) as any;expect(ownerView.participants.map((p:any)=>p.email)).toEqual(['a@example.com','b@example.com']);expect(ownerView.organizerMail).toEqual({used:0,limit:5});
  await expect(sendOrganizerMail(env,e.id,user('other'),{participantIds:['a'],subject:'集合',message:'周六见'},'other')).rejects.toMatchObject({status:403});
  for(let n=1;n<=5;n++){const result=await sendOrganizerMail(env,e.id,user('owner'),{participantIds:n===1?['a']:['a','b'],subject:`通知 ${n}`,message:'请查看活动安排'},`mail-${n}`);expect(result.used).toBe(n);}
  expect(await sendOrganizerMail(env,e.id,user('owner'),{participantIds:['a'],subject:'通知 1',message:'不会重复'},'mail-1')).toEqual({sent:1,used:5,limit:5});
