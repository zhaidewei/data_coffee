...
describe('D1事务与公开投影',()=>{
 ...
 it('扩容通过 CAS 持久化一次并写入类型化通知',async()=>{
   let e=await published();for(const id of ['a','b','c','d'])e=await execute(env,e.id,{action:'join'},user(id),`capacity-join-${id}`);
   const expanded=await execute(env,e.id,{action:'raise_capacity',maxPeople:4},user('owner'),'capacity-expand',e.version);
   expect(expanded.rules.maxPeople).toBe(4);expect(expanded.receipts.at(-1)?.capacityChange).toEqual({before:3,after:4});
   const repeated=await execute(env,e.id,{action:'raise_capacity',maxPeople:4},user('owner'),'capacity-expand',e.version);
   expect(repeated.version).toBe(expanded.version);
   const audit=await env.DB.prepare("SELECT count(*) n FROM audit WHERE event_id=? AND action='raise_capacity'").bind(e.id).first<{n:number}>();expect(audit?.n).toBe(1);
   const notices=await env.DB.prepare("SELECT DISTINCT kind,priority FROM outbox WHERE kind='capacity_expanded'").all();expect(notices.results).toEqual([{kind:'capacity_expanded',priority:1}]);
 });
