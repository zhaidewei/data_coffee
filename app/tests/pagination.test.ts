...
const list=(query='',actor:User|null=null,clock=now)=>listEvents(env,actor,new URLSearchParams(query),()=>clock);
...
describe('活动服务端分页',()=>{
 ...
 it('未到期活动只需一次文档批量查询，卡片增加不增加逐条查询',async()=>{
  await seed(Array.from({length:25},(_,i)=>item(i)));const spy=vi.fn((sql:string)=>env.DB.prepare(sql));const counted={...env,DB:{prepare:spy} as unknown as D1Database};await listEvents(counted,null,new URLSearchParams(),()=>now);expect(spy).toHaveBeenCalledTimes(1);spy.mockClear();await listEvents(counted,null,new URLSearchParams('pageSize=20'),()=>now);expect(spy).toHaveBeenCalledTimes(1);
 });
 it('到期活动在响应前推进，不返回已过期的recruiting决策',async()=>{
  await seed([item(0)]);const r=await list('',null,rules.recruitmentDeadline+1);expect(r.events[0].status).toBe('cancelled');const stored=await env.DB.prepare('SELECT document FROM activities WHERE id=?').bind('event-000').first<{document:string}>();expect(JSON.parse(stored!.document).status).toBe('cancelled');expect((await env.DB.prepare("SELECT count(*) n FROM audit WHERE action='reconcile'").first<{n:number}>())!.n).toBe(1);
 });
