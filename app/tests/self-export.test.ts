...
const now=Date.parse('2026-09-09T08:00:00Z'),rawToken='dcf_'+'a'.repeat(64);
...
async function seed(){
  ...
  await env.DB.prepare('INSERT INTO personal_tokens(id,user_id,name,scope,token_hash,created_at,expires_at) VALUES (?,?,?,?,?,?,?)').bind('token','mine','导出','read',await hash(rawToken),now,now+86400000).run();
  ...
  await env.DB.batch([
    ...
    env.DB.prepare("INSERT INTO auth_sessions(token_hash,user_id,expires_at) VALUES ('private-session-hash','mine',?)").bind(now+86400000),
...
it('认证用户可导出本人完整关系，且不会导出凭证和其他成员隐私',async()=>{
  ...
  expect(data.audit).toHaveLength(2);expect(data.notifications).toHaveLength(1);expect(data.tokens[0]).toMatchObject({id:'token',scope:'read'});expect(data.sessions).toEqual([{expiresAt:now+86400000}]);expect(data.aiProposals).toHaveLength(1);
