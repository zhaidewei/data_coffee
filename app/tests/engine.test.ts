...
describe('确定性业务规则',()=>{
 it('创建和编辑时把已知城市别名统一为目录名称',()=>{const e=createActivity({title:'城市别名',city:'The Hague',rules:rules()},'owner',t);expect(e.city).toBe('Den Haag');applyCommand(e,{action:'edit',title:e.title,city:'Den Bosch',rules:e.rules},'owner',t,[]);expect(e.city).toBe('’s-Hertogenbosch');const custom=createActivity({title:'自填城市',city:'Nieuwe Stad',rules:rules()},'owner',t);expect(custom.city).toBe('Nieuwe Stad');});
...
it('时段投票不能替代最终确认，人数按最终时段计算',()=>{const e=event({timeSlots:slots});act(e,'join','a',t+1000,{availableSlotIds:['sat']});act(e,'join','b',t+1100,{availableSlotIds:['sun']});expect(conditions(e).find(c=>c.key==='people')?.current).toBe(0);expect(()=>act(e,'select_time','a',t+1200,{slotId:'sat'})).toThrow();act(e,'select_time','owner',t+1300,{slotId:'sun'});expect(e.rules.startsAt).toBe(slots[1].startsAt);expect(e.rules.recruitmentDeadline).toBe(slots[1].startsAt-3600000);expect(e.rules.registrationDeadline).toBe(slots[1].startsAt-(slots[0].startsAt-rules().registrationDeadline));expect(e.rules.promotionDeadline).toBe(slots[1].startsAt-(slots[0].startsAt-rules().promotionDeadline));expect(conditions(e).find(c=>c.key==='people')?.current).toBe(1);expect(e.participants[0].status).toBe('left');expect(()=>act(e,'select_time','owner',t+1400,{slotId:'sat'})).toThrow();reconcile(e,e.rules.recruitmentDeadline,[]);expect(e.status).toBe('cancelled');});
it('未确认时间时逐个淘汰候选日期，最后一个期限才取消活动',()=>{
 const progressive=[slots[0],{id:'middle',startsAt:t+26*3600000,endsAt:t+29*3600000},{id:'late',startsAt:t+50*3600000,endsAt:t+53*3600000}];
 const e=event({timeSlots:progressive});
 act(e,'join','first-only',t+1000,{availableSlotIds:['sat']});
 act(e,'join','later',t+1100,{availableSlotIds:['sat','middle','late']});
 const first:Notice[]=[];reconcile(e,t+3600000,first);
 expect(e.status).toBe('recruiting');expect(e.rules.timeSlots?.map(s=>s.id)).toEqual(['middle','late']);
 expect(e.rules.recruitmentDeadline).toBe(t+25*3600000);expect(nextDue(e)).toBe(t+25*3600000);
 expect(e.participants.find(p=>p.userId==='first-only')?.status).toBe('left');
 expect(e.participants.find(p=>p.userId==='later')?.availableSlotIds).toEqual(['middle','late']);
 expect(first).toContainEqual(expect.objectContaining({userId:'first-only',kind:'candidate_time_expired',priority:0,deliverBefore:t+25*3600000}));
 reconcile(e,t+25*3600000,[]);expect(e.status).toBe('recruiting');expect(e.rules.timeSlots?.map(s=>s.id)).toEqual(['late']);expect(e.rules.recruitmentDeadline).toBe(t+49*3600000);
 reconcile(e,t+49*3600000,[]);expect(e.status).toBe('cancelled');expect(e.receipts.filter(r=>r.kind==='candidate_time_expired')).toHaveLength(2);
});
it('未确认时间即使投票充足也只在最后一个候选期限取消',()=>{const e=event({timeSlots:slots});for(const u of ['a','b','c'])act(e,'join',u,t+1000,{availableSlotIds:['sat','sun']});reconcile(e,t+3600000,[]);expect(e.status).toBe('recruiting');reconcile(e,e.rules.recruitmentDeadline,[]);expect(e.status).toBe('cancelled');});
it('一次错过全部候选期限时只发送取消通知',()=>{const e=event({timeSlots:slots});act(e,'join','first-only',t+1000,{availableSlotIds:['sat']});const out:Notice[]=[];reconcile(e,slots[1].startsAt-3600000,out);expect(e.status).toBe('cancelled');expect(e.participants[0].status).toBe('joined');expect(out.some(n=>n.kind==='candidate_time_expired')).toBe(false);expect(out).toContainEqual(expect.objectContaining({userId:'first-only',kind:'activity_cancelled'}));});
...
it('乱序候选输入按开始时间规范化后逐期推进',()=>{const ordered=[slots[0],slots[1],{id:'late',startsAt:t+2*86400000,endsAt:t+2*86400000+3600000}],e=event({timeSlots:[ordered[1],ordered[0],ordered[2]]});expect(e.rules.timeSlots?.map(s=>s.id)).toEqual(['sat','sun','late']);reconcile(e,t+3600000,[]);expect(e.rules.timeSlots?.map(s=>s.id)).toEqual(['sun','late']);});
