...
beforeAll(async()=>{
 ...
 for(const f of ['0001_events.sql','0002_identity.sql','0006_mail_digest.sql','0008_outbox_notice_kind.sql','0009_organizer_mail.sql','0004_tags.sql'])for(const sql of readFileSync(new URL(`../migrations/${f}`,import.meta.url),'utf8').split(';').filter(s=>s.trim()))await DB.prepare(sql).run();
});
