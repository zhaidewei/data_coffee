...
import {openEventMessages} from './event-messages.js';
...
function renderDetail(e){
  ...
  const parallel=el('section',{class:'parallel-lanes','aria-label':'并行参与阶段'},
    ...
    el('div',{class:'parallel-lane'},el('span',{class:'lane-label'},'并行 B'),node('提议场地',appsFor(['venue']).filter(a=>a.status==='pending').length+' 个候选场地',active&&beforeStart,'参与',contributions(['venue'],'尚无场地提议。')),arrow(),node('发起人确认场地',venueCondition?.satisfied?'场地已确认':e.rules.venueRequired?'等待确认场地':'场地非必需',active&&beforeStart&&(e.isOwner||joined==='joined'),e.isOwner?'确认':joined==='joined'?'投票':'查看',el('div',{},el('p',{class:'muted'},'已获得名额的成员可为候选场地投票并改选；票数供参考，由发起人最终确认。'),appsFor(['venue']).length?appsFor(['venue']).map(a=>applicationNode(a,e,true,false)):el('p',{class:'muted'},'还没有候选场地。')))),
  ...
  const nodeTitles=['活动规则','选时间报名 · 点击这里','确认时间','提议场地 · 点这里','确认场地 · 成员投票','帮忙组织 · 点这里','确认帮忙人选','成行检查'];
  const nodeSummaries=[
  ...
  const registrationButton=btn(['joined','waitlisted'].includes(joined)?'查看我的报名':'选时间报名点击这里',()=>{
