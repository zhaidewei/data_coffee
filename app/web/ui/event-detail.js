...
function raiseCapacityForm(e){
 ...
 f.onsubmit=async ev=>{ev.preventDefault();submit.disabled=true;try{await command('raise_capacity',{maxPeople:Number(input.value)});}catch(err){errorAt(f,err);submit.disabled=false;}};
 modal(f);
}
...
function renderDetail(e){
  ...
  if(e.isOwner&&!['cancelled','completed'].includes(e.status)){
    ...
    if(e.status==='draft')row.append(btn('编辑草稿',()=>renderEventForm(e)),btn('预览并发布',()=>publishPreview(e),'button dark'));
      ...
      row.append(btn('更正活动介绍',()=>{const tags=tagPicker(e.tags||[]);const f=el('form',{},el('h2',{},'更正活动介绍'),tags.root,field('介绍','description','textarea',e.description),el('button',{class:'button dark',type:'submit'},'保存更正'));f.onsubmit=async ev=>{ev.preventDefault();try{await command('describe',{description:f.elements.description.value,tags:tags.read()});}catch(err){errorAt(f,err);}};modal(f);}));
      if(Date.now()<e.rules.registrationDeadline&&['recruiting','confirmed','repairing'].includes(e.status)&&e.rules.maxPeople<100)row.append(btn('扩充名额',()=>raiseCapacityForm(e),'button quiet'));
    }
