...
async function login(onAuthenticated){const form=el('form',{},el('span',{class:'eyebrow'},'WELCOME TO THE TABLE'),el('h2',{},'登录 Data Coffee'),el('p',{class:'muted'},'使用邮箱验证码登录。首次登录验证成功后，再设置昵称。'),field('邮箱','email','email'));const code=field('验证码','code','text');code.hidden=true;code.querySelector('input').required=false;const submit=el('button',{class:'button dark',type:'submit'},'发送验证码');form.append(code,submit);let sent=false;form.onsubmit=async e=>{e.preventDefault();submit.disabled=true;try{const v=Object.fromEntries(new FormData(form));if(!sent){const r=await api('/api/auth/request',{email:v.email});sent=true;code.hidden=false;code.querySelector('input').required=true;form.elements.email.readOnly=true;submit.textContent='验证并登录';if(r.developmentCode)form.append(el('p',{class:'form-note'},`开发环境验证码：${r.developmentCode}`));else toast('验证码已发送，请检查邮箱。');code.querySelector('input').focus();}else{const r=await api('/api/auth/verify',v);state.user=r.user;updateAccount();closeModal();await route();const resumed=await onAuthenticated?.();if(!state.user.nickname&&resumed!==false)setupNickname();else if(!onAuthenticated)toast('登录成功，欢迎回来。');}}catch(err){errorAt(form,err);}finally{submit.disabled=false;}};modal(form);}
function requireUser(onAuthenticated){if(state.user)return true;login(onAuthenticated);return false;}
