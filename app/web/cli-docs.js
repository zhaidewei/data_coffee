...
<h3>参数</h3><ul><li><code>--data</code>：JSON 对象、@文件，或 - 从 stdin 读取。</li><li><code>--version</code>：读取到的当前非负整数版本。</li><li><code>--key</code>：一次动作的幂等键，1–100 个字母、数字、冒号、下划线或连字符。</li><li><code>--session-stdin</code>：从 stdin 读取会话；不能与 --data - 同用。</li><li><code>--session-only</code>：仅用于 auth verify，输出 {sessionToken}，供安全管道捕获。</li></ul></section>
<section id="auth"><h2>认证与会话</h2><p>使用邮箱验证码登录。会话通过 <code>DATA_COFFEE_SESSION</code> 或 <code>--session-stdin</code> 注入；stdin 接受 64 位十六进制 token 或 {sessionToken} JSON。CLI 不保存凭证，也不接受命令行 token。</p><p>Agent 应在调用点通过本地 secret CLI 读取会话并管道注入，避免将会话或验证码写入命令参数、日志和文件。验证登录时，用 stdin 传入验证码请求体，并捕获 --session-only 输出。</p></section>
...
# 将 VERSION 替换为刚读取的版本；会话由运行环境安全注入
...
<p>CLI 不自动重试。重试同一次 action 必须复用原始 key、version 和请求体；业务意图改变时使用新 key。创建草稿没有幂等支持，超时后先查列表，避免重复创建。</p><p>请求超时 30 秒，拒绝重定向。目前没有分页、删除活动、自动刷新登录或独立 Agent token。</p></section></article>`;
