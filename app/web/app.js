...
function overviewCardVisual(e,slots){const calendar=overviewCalendar(slots),root=el('div',{class:'overview-card-visual'},calendar);if(calendar.children.length<2){const image=el('img',{class:'city-coffee',crossorigin:'anonymous',src:cityCoffee(e.city)+'?cors=1',alt:'',loading:'lazy'});image.onerror=()=>{if(image.getAttribute('src')!==defaultCoffee)image.src=defaultCoffee;else image.hidden=true;};const info=el('div',{class:'overview-card-info'},el('div',{class:'overview-story'},image,el('p',{class:'overview-description'},e.description)),eventProgress(e));root.append(info);root.classList.add('single-month');}else root.append(el('div',{class:'overview-card-info'},el('p',{class:'overview-description'},e.description),eventProgress(e)));return root;}
...
function decisionSummary(e){
  ...
  const root=el('section',{class:'event-decision decision-'+e.status,'aria-label':'当前活动决策'},
    ...
    el('dl',{class:'decision-facts'},
      item('时间',fixed?date(fixed.startsAt)+' — '+date(fixed.endsAt):'待确认'),
      item('地点',e.city+(venues.length?' · '+venues.map(v=>v.title+(v.address?' · '+v.address:'')).join('、'):e.rules.venueRequired?' · 场地待确认':' · 具体地点待约定')),
      item('参与人数',(e.counts?.joined||0)+' 人报名 · '+e.rules.minPeople+' 人起成行'),
      item('现场负责人',hosts?hosts.current>0?'已确认 '+hosts.current+' 人':'待确认':'未要求')));
