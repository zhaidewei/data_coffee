...
function eventProgress(e){const cs=(e.conditions||[]).filter(c=>c.required>0),done=cs.filter(c=>c.satisfied).length,total=cs.length;if(!total)return document.createDocumentFragment();const label=(['completed','cancelled'].includes(e.status)?'最后条件快照':'成行条件')+' · '+done+'/'+total+' 项已满足';return el('div',{class:'event-progress'},el('div',{},label),el('progress',{max:total,value:done,'aria-label':label}));}
...
function decisionSummary(e,openNode){
  if(e.status==='draft')return null;
  const titles={recruiting:'尚未成行 · 正在征集',confirmed:'已成行',repairing:'成行条件有变化 · 等待补齐',cancelled:'活动已取消',completed:'活动已结束'};
  const selected=(e.rules.timeSlots||[]).find(s=>s.id===e.selectedSlotId);
  const fixed=selected||(!(e.rules.timeSlots||[]).length?e.rules:null);
  const venues=(e.applications||[]).filter(a=>a.kind==='venue'&&a.status==='approved');
  const hosts=(e.conditions||[]).find(c=>c.key==='hosts');
  const missing=(e.conditions||[]).filter(c=>!c.satisfied);
  const terminal=['completed','cancelled'].includes(e.status);
  const note=e.status==='completed'?'活动周期已结束，以下为最后确认的安排。':e.status==='cancelled'?(e.reason||'本场活动已取消，请勿按原安排前往。'):e.status==='confirmed'?'当前已满足成行规则，以下为已确认安排。':e.status==='repairing'?'当前条件不足，请等待补齐后的最新决策。':'发布与报名不代表成行，请以这里的最新决策为准。';
  const item=(name,value,hint,action,index)=>el('div',{},el('dt',{},name),el('dd',{},value,hint?el('p',{class:'decision-guidance'},hint):null,!terminal&&action?btn(action,()=>openNode(index),'button quiet'):null));
  const root=el('section',{class:'event-decision decision-'+e.status,'aria-label':'当前活动决策'},
    el('span',{class:'eyebrow'},terminal?'最终状态':'当前决策'),
    el('h2',{},titles[e.status]||statusNames[e.status]),el('p',{},note),
    el('dl',{class:'decision-facts'},
      item('时间',fixed?date(fixed.startsAt)+' — '+date(fixed.endsAt):'待确认',!fixed?'由发起人在 '+date(e.rules.recruitmentDeadline)+' 前，从候选时段中确认一次。请先勾选你能参加的所有日期。':null,!fixed?(e.isOwner?'查看偏好并确认时间':'选择可参加日期'):null,e.isOwner?2:1),
      item('地点',e.city+(venues.length?' · '+venues.map(v=>v.title+(v.address?' · '+v.address:'')).join('、'):e.rules.venueRequired?' · 场地待确认':' · 具体地点待约定'),!venues.length?(e.rules.venueRequired?'成员可推荐场地，由发起人审核；容量须至少容纳 '+e.rules.maxPeople+' 人。':'场地不作为成行条件。欢迎推荐咖啡馆，由发起人确认具体地点。'):null,!venues.length?'查看或推荐场地':null,3),
      item('参与人数',(e.counts?.joined||0)+' 人报名 · '+e.rules.minPeople+' 人起成行','最终确认时段须有至少 '+e.rules.minPeople+' 人可参加，意向报名总数不等于该时段人数。',!terminal?'查看报名与偏好':null,1),
      item('现场负责人',hosts?hosts.current>0?'已确认 '+hosts.current+' 人':'待确认':'未要求',hosts&&hosts.current<hosts.required?'需 '+hosts.required+' 位成员自愿报名并由发起人确认。':'本项'+(hosts?'已满足。':'不影响成行。'),hosts&&hosts.current<hosts.required?'报名现场负责人':null,5)));
  root.append(eventTags(e),eventProgress(e));
  if(venues.some(v=>!v.address)&&e.rules.addressVisibility==='participants')root.append(el('small',{},'详细地址按活动规则向参与者展示。'));
  if(!terminal&&e.status!=='confirmed'){
    root.append(el('p',{class:'decision-pending'},missing.length?'待满足：'+missing.map(c=>(c.key==='people'?'最终时段可参加人数':c.label)+'（'+c.current+'/'+c.required+'）').join(' · '):'条件已满足，等待系统按规则确认成行。'));
    const deadline=e.status==='repairing'&&e.repairs?.length?Math.min(...e.repairs.map(r=>r.deadline).filter(Number.isFinite)):e.rules.recruitmentDeadline;
    if(Number.isFinite(deadline))root.append(el('p',{class:'decision-deadline'},(e.status==='repairing'?'补齐截止：':'征集截止：')+date(deadline)));
  }
  return root;
}
function renderDetail(e){
  ...
  const nodeTitles=['活动规则','报名','确认时间','提议场地','确认场地','报名负责人','确认负责人','成行检查'];
  const nodeSummaries=[
  ...
  app.replaceChildren(el('a',{class:'back',href:'#'},'← 所有聚会'),el('header',{class:'detail-header code-header'},el('span',{class:'eyebrow'},'COFFEE DAG / GRAPH'),el('h1',{},e.title),el('p',{class:'muted publisher'},'发布人：'+(e.publisher?.nickname||'匿名成员')),decisionSummary(e,index=>nodes[index]._dagButton.click()),el('div',{class:'detail-primary-actions'},badge(e.status),e.status!=='draft'?registrationButton:null,e.status!=='draft'?btn('分享活动',()=>shareEvent(e),'button share-cta'):null)),draft?el('div',{class:'draft-toolbar'},el('span',{},'草稿预览 · 仅你可见'),btn('返回编辑',()=>renderEventForm(e)),btn('发布活动',()=>publishPreview(e),'button dark'),btn('删除草稿',()=>deleteDraftDialog(e),'button danger')):document.createDocumentFragment(),el('div',{class:'dag-workspace dag-modal-workspace'},el('section',{class:'dag-graph'},el('div',{class:'dag-toolbar'},'点击节点，查看详情或办理事项 · 连线表示活动流程'),canvas,el('p',{class:'muted'},'报名 / 提议 → 发起人确认 → READY · 截止后按规则成行或取消'))));
}
