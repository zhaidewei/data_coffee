...
function applicationNode(a,e){const own=(e.myApplications||[]).some(x=>x.id===a.id);const mutable=!['cancelled','completed'].includes(e.status)&&Date.now()<e.rules.startsAt;const canReview=mutable&&(a.kind==='cohost'?e.isOwner:e.canManage);return el('div',{class:'application'},el('h3',{},`${kinds[a.kind]||a.kind} · ${a.title} `,badge(a.status)),el('p',{},a.detail),a.capacity?el('p',{},`容量：${a.capacity} 人`):null,a.address?el('p',{},`地址：${a.address}`):null,a.duration?el('p',{},`分享时长：${a.duration} 分钟`):null,a.amount?el('p',{},`赞助：€${a.amount}`):null,a.reason?el('p',{},`处理说明：${a.reason}`):null,el('div',{class:'inline-actions'},mutable&&own&&['pending','approved'].includes(a.status)?btn('撤回 / 退出',()=>confirmation('撤回这项申请？','撤回已通过的申请可能影响成行条件。','withdraw',{applicationId:a.id}),'button quiet'):null,canReview&&a.status==='pending'?btn('通过',()=>confirmation('通过申请？',`批准「${a.title}」，系统将重新检查成行条件。`,'review',{applicationId:a.id,approved:true})):null,canReview&&a.status==='pending'?btn('拒绝',()=>confirmation('拒绝申请','请向申请人说明原因。','review',{applicationId:a.id,approved:false},true),'button quiet'):null,mutable&&e.isOwner&&a.kind==='cohost'&&a.status==='approved'?btn('撤销协办资格',()=>confirmation('撤销批准','撤销后系统将重新检查活动条件。','revoke',{applicationId:a.id},true),'button danger'):null));}
...
function renderDetailPrevious(e){
  ...
  joinDetail.append(el('details',{id:'detail-participants'},el('summary',{},'查看参与名单'),people.length?el('div',{class:'participant-list'},people.map(x=>el('span',{class:'person'},`${x.nickname||'匿名参与者'}${x.isMe?'（我）':''} · ${statusNames[x.status]||x.status}`))):el('p',{class:'muted'},'还没有人报名。')),el('div',{class:'inline-actions'},btn('问问活动助手 ✳',()=>openAI(),'button')));
...
function renderDetail(e){
  ...
  const contributions=(ks,empty)=>el('div',{},active&&beforeStart?el('div',{class:'inline-actions'},ks.map(k=>btn(`＋ ${k==='host'?'报名主持人':'提供场地'}`,()=>applyForm(k)))):null,
    appsFor(ks).length?appsFor(ks).map(a=>applicationNode(a,e)):el('p',{class:'muted'},empty));
  const summaryChart=(title,items=[])=>{
    const max=Math.max(1,...items.map(x=>x.count));
    return el('div',{class:'preference-chart'},el('strong',{},title),items.length?items.map(x=>el('div',{class:'preference-row'},el('span',{},x.label),el('i',{},el('b',{style:`width:${Math.max(8,x.count/max*100)}%`})),el('em',{},x.count))):el('p',{class:'muted'},'等待第一份偏好'));
  };
  ...
  const registrationOpen=active&&beforeStart&&(Date.now()<e.rules.registrationDeadline||!!repair);
  const waitOpen=e.rules.waitlist&&Date.now()<e.rules.promotionDeadline;
  const prefForm=el('form',{class:'preference-form'},field('可参加时间','timePreference','text',p?.timePreference||date(e.rules.startsAt)),field('地点偏好','placePreference','text',p?.placePreference||e.city));
  const prefSubmit=el('button',{class:'button orange',type:'submit'},joined&&joined!=='left'?'保存偏好':'报名并保存偏好');
  prefForm.append(el('p',{class:'form-note'},'填写你的偏好，发布者根据汇总确定最终安排。'),prefSubmit);
  prefForm.onsubmit=async ev=>{ev.preventDefault();prefSubmit.disabled=true;try{await command('join',{timePreference:prefForm.elements.timePreference.value,placePreference:prefForm.elements.placePreference.value});}catch(err){errorAt(prefForm,err);}finally{prefSubmit.disabled=false;}};
  ...
  const joinDetail=el('div',{class:'registration-detail'},el('div',{class:'registration-actions'},el('p',{class:'muted'},`最低 ${e.rules.minPeople} 人，最多 ${e.rules.maxPeople} 人；报名截止：${date(e.rules.registrationDeadline)}`),
    registrationOpen&&(!full||waitOpen||['joined','waitlisted'].includes(joined))?prefForm:el('p',{class:'form-note'},'报名已关闭。'),
    ...
    el('details',{id:'detail-participants'},el('summary',{},'查看参与名单'),people.length?el('div',{class:'participant-list'},people.map(x=>el('span',{class:'person'},`${x.nickname||'匿名参与者'}${x.isMe?'（我）':''} · ${statusNames[x.status]||x.status}`))):el('p',{class:'muted'},'还没有人报名。'))),
    el('div',{class:'preference-summary'},summaryChart('时间汇总',prefSummary.times),summaryChart('地点汇总',prefSummary.places)));
  ...
  const parallel=el('section',{class:'parallel-lanes','aria-label':'并行参与阶段'},
    el('div',{class:'parallel-lane'},el('span',{class:'lane-label'},'并行 A'),node('报名并提交偏好',`${e.counts?.joined||0}/${e.rules.minPeople} 人${joined&&joined!=='left'?' · 我'+statusNames[joined]:''}`,registrationOpen,'参与',joinDetail,e.status==='recruiting'),arrow(),node('确定时间与地点',`${date(e.rules.startsAt)} · ${e.city}`,false,'查看',el('div',{},el('p',{class:'journey-lead'},`${date(e.rules.startsAt)} — ${date(e.rules.endsAt)}`),el('p',{class:'muted'},'发布者根据报名偏好确认；当前显示已发布安排。')))),
    el('div',{class:'parallel-lane'},el('span',{class:'lane-label'},'并行 B'),node('提供场地',e.rules.venueRequired?(venueCondition?.satisfied?'场地已确认':`待确认 · ${venueCondition?.current||0}/${venueCondition?.required||e.rules.maxPeople}`):'本场不强制要求场地',active&&beforeStart,'参与',contributions(['venue'],'尚无场地申请。'))),
    el('div',{class:'parallel-lane'},el('span',{class:'lane-label'},'并行 C'),node('报名现场活动主持人',e.rules.minHosts?(hostCondition?.satisfied?'主持人已确认':`待确认 · ${hostCondition?.current||0}/${hostCondition?.required||e.rules.minHosts}`):'本场不强制要求主持人',active&&beforeStart,'参与',contributions(['host'],'尚无主持人申请。'))));
  ...
  const extraConditions=conditions.filter(c=>!['people','venue','hosts'].includes(c.key));
  if(extraConditions.length)readyDetail.append(el('p',{class:'form-note'},'本场发布者还设定了分享、协办或角色条件，以上状态也会参与 READY 判断。'),active&&beforeStart?el('div',{class:'inline-actions'},btn('＋ 申请分享',()=>applyForm('talk')),btn('＋ 申请协办',()=>applyForm('cohost'))):null);
...
function publishPreview(e){const f=el('form',{},el('h2',{},'发布前，最后看一眼'),el('div',{class:'preview'},el('h3',{},e.title),el('p',{},`${e.city} · ${date(e.rules.startsAt)}`),el('p',{class:'prose'},e.description),el('p',{},`成行最低 ${e.rules.minPeople} 人，最多 ${e.rules.maxPeople} 人`),el('p',{},`征集截止：${date(e.rules.recruitmentDeadline)}`),el('p',{},`结束：${date(e.rules.endsAt)} · 报名截止：${date(e.rules.registrationDeadline)} · 递补截止：${date(e.rules.promotionDeadline)}`),el('p',{},`场地${e.rules.venueRequired?'必需':'非必需'} · 分享 ${e.rules.minTalks} · 协办 ${e.rules.minCohosts} · 现场负责人 ${e.rules.minHosts}`),el('p',{},`${e.rules.waitlist?'允许候补':'不开放候补'} · ${e.rules.allowRoleOverlap?'允许角色兼任':'角色不可兼任'} · 补齐 ${e.rules.repairMinutes} 分钟`),el('p',{},`持续检查：${[['continuousVenue','场地'],['continuousTalks','分享'],['continuousCohosts','协办'],['continuousHosts','现场负责人']].filter(([k])=>e.rules[k]).map(([,v])=>v).join('、')||'无'} · 地址${e.rules.addressVisibility==='public'?'公开':'仅参与者可见'}`)),el('p',{class:'form-note'},'发布后，人数、截止时间与其他成行规则将锁定。活动创建者不会自动报名，也不会自动成为现场负责人。'),el('button',{type:'submit',class:'button orange'},'确认规则并发布'));f.onsubmit=async ev=>{ev.preventDefault();const b=f.querySelector('button[type=submit]');b.disabled=true;try{await command('publish');}catch(err){errorAt(f,err);}finally{b.disabled=false;}};modal(f);}
...
function timeField(label,name,value){const input=el('input',{id:name,name,type:'datetime-local',required:true,value:localParts(value)});const offset=el('select',{name:`${name}Offset`,'aria-label':`${label}重复时间选择`},el('option',{value:'early'},'首次（夏令时优先）'),el('option',{value:'late'},'第二次（冬令时）'));if(amsterdamMs(localParts(value),'late')!==amsterdamMs(localParts(value),'early')&&amsterdamMs(localParts(value),'late')===Math.floor(value/60000)*60000)offset.value='late';return el('label',{class:'field'},label,el('div',{class:'time-row'},input,offset));}
...
function renderEventForm(existing){state.detailVisible=false;state.pendingEvent=null;if(!requireUser())return;const day=86400000, start=Date.now()+14*day;const defaults={minPeople:10,maxPeople:12,waitlist:true,recruitmentDeadline:start-2*day,startsAt:start,endsAt:start+2*3600000,registrationDeadline:start-3600000,promotionDeadline:start-3600000,repairMinutes:60,venueRequired:true,minTalks:1,minCohosts:0,minHosts:1,allowRoleOverlap:true,continuousVenue:true,continuousTalks:false,continuousCohosts:false,continuousHosts:true,addressVisibility:'participants'};const r=existing?.rules||defaults;const f=el('form',{class:'wide-form'},el('a',{class:'back',href:existing?`#event/${existing.id}`:'#',onclick:existing?e=>{e.preventDefault();renderDetail(existing);}:undefined},'← 返回'),el('span',{class:'eyebrow'},'MAKE ROOM FOR A GOOD CONVERSATION'),el('h1',{},existing?'编辑聚会草稿':'从一个想聊的话题开始'),el('p',{class:'muted'},'先保存为草稿，再预览发布。把成行规则讲清楚，让每个人都知道自己的承诺。'));f.append(el('div',{class:'panel'},field('聚会标题','title','text',existing?.title||''),field('城市','city','text',existing?.city||'Amsterdam'),field('聚会介绍：聊什么，适合谁，如何安排','description','textarea',existing?.description||'')));const times=el('fieldset',{},el('legend',{},'01 / 相聚时间'),el('p',{class:'form-note'},'全部输入均为荷兰 Europe/Amsterdam 时间，与你当前设备时区无关。十月切换冬令时的重复时段可选择第一次或第二次。'),el('div',{class:'form-grid'},timeField('开始时间','startsAt',r.startsAt),timeField('结束时间','endsAt',r.endsAt),timeField('征集截止','recruitmentDeadline',r.recruitmentDeadline),timeField('报名截止','registrationDeadline',r.registrationDeadline),timeField('递补截止','promotionDeadline',r.promotionDeadline)));const rules=el('fieldset',{},el('legend',{},'02 / 固定成行条件'),el('p',{class:'muted'},'这些规则在发布后锁定。现场负责人、协办与分享均需本人申请并通过审核。'),el('div',{class:'form-grid'},field('最低成行人数','minPeople','number',r.minPeople),field('最大人数','maxPeople','number',r.maxPeople),field('最少分享数','minTalks','number',r.minTalks),field('最少协办人数','minCohosts','number',r.minCohosts),field('最少现场负责人数','minHosts','number',r.minHosts),field('条件补齐窗口（分钟）','repairMinutes','number',r.repairMinutes)),check('满员后允许候补','waitlist',r.waitlist),check('成行需要已确认场地，容量覆盖人数上限','venueRequired',r.venueRequired),check('允许同一人兼任不同角色','allowRoleOverlap',r.allowRoleOverlap));const continuous=el('fieldset',{},el('legend',{},'03 / 成行后持续检查'),check('场地持续满足','continuousVenue',r.continuousVenue),check('分享持续满足','continuousTalks',r.continuousTalks),check('协办持续满足','continuousCohosts',r.continuousCohosts),check('现场负责人持续满足','continuousHosts',r.continuousHosts),el('label',{class:'field'},'场地地址可见范围',el('select',{name:'addressVisibility'},el('option',{value:'participants',selected:r.addressVisibility==='participants'},'有权限的活动参与者'),el('option',{value:'public',selected:r.addressVisibility==='public'},'所有人'))));const save=el('button',{class:'button dark',type:'submit'},existing?'保存草稿':'保存草稿，进入预览 →');f.append(times,rules,continuous,save);f.querySelectorAll('input[type=number]').forEach(n=>{n.min=['minTalks','minCohosts','minHosts'].includes(n.name)?'0':'1';n.step='1';});f.onsubmit=async ev=>{ev.preventDefault();save.disabled=true;try{const fd=new FormData(f);const rr={};for(const [k,v] of Object.entries(r)){if(typeof v==='boolean')rr[k]=fd.has(k);else if(['startsAt','endsAt','recruitmentDeadline','registrationDeadline','promotionDeadline'].includes(k))rr[k]=amsterdamMs(fd.get(k),fd.get(k+'Offset'));else if(typeof v==='number')rr[k]=Number(fd.get(k));else rr[k]=fd.get(k);}if(!(rr.recruitmentDeadline<rr.startsAt&&rr.startsAt<rr.endsAt))throw new Error('请确保征集截止早于开始时间，结束时间晚于开始时间。');if(['registrationDeadline','promotionDeadline'].some(k=>rr[k]<rr.recruitmentDeadline||rr[k]>rr.startsAt))throw new Error('报名和递补截止须在征集截止与活动开始之间。');if(rr.minPeople>rr.maxPeople)throw new Error('最低人数不能超过人数上限。');const data={title:fd.get('title').trim(),city:fd.get('city').trim(),description:fd.get('description').trim(),rules:rr};if(existing){await command('edit',data);}else{const response=await api('/api/events',data);location.hash=`event/${response.event.id}`;toast('草稿已保存，请预览并确认发布。');}}catch(err){errorAt(f,err);}finally{save.disabled=false;}};app.replaceChildren(f);window.scrollTo({top:0});}
...
async function pollDetail(){
  ...
  try{
    ...
    for(const k of ['canManage','isOwner','myParticipation','myApplications','conditions','counts','participants','applications','receipts'])if(result[k]!==undefined)next[k]=result[k];
