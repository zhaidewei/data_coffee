...
export async function listEvents(env:Env,user:User|null,params:URLSearchParams){
  ...
  const range=rangeOf(params),requestedCity=params.get('city')||'',city=canonicalCity(requestedCity)||requestedCity,tag=(params.get('tag')||'').toLowerCase();
  ...
  const cityName=(e:Activity)=>canonicalCity(e.city)||e.city;
  const matches=themed.filter(e=>!city||city==='全部'||cityName(e)===city);
  const cityCounts=new Map<string,number>();for(const e of themed){const name=cityName(e);cityCounts.set(name,(cityCounts.get(name)||0)+1);}
  ...
  for(const e of dated.filter(e=>!city||city==='全部'||cityName(e)===city))for(const key of new Set((e.tags||[]).map(t=>t.toLowerCase())))tagCounts.get(key)!.count++;
  ...
  return {events,user:user?{id:user.id,nickname:user.nickname,publicNickname:user.publicNickname}:null,
    ...
    overview:{total:themed.length,cities:[...cityCounts].map(([city,count])=>({city,count})),tags:[...tagCounts.values()].sort((a,b)=>a.label.localeCompare(b.label,'zh-CN')),allCities:[...new Set(visible.map(cityName))]}};
}
