...
- 单一或已确认时段以该时段的成行期限判断全部条件。多个尚未确认的候选日期按相同提前量逐期截止：前序日期未确认只移除该日期，最后日期仍未确认才取消。执行延迟不能把各自期限后才生效的变更计入对应判定（D11）。
...
单一或已确认时段的全部成团条件按该时段成行期限的有效状态统一判断（D11 已确认），涵盖报名与退出、资源核实及撤回等。多个尚未确认的候选日期按首个日期设置的提前量逐期截止；前序日期未确认只移除该日期并继续征集，最后日期仍未确认才取消。执行延迟不能将期限后生效的变更算入对应判定；截止边界的精确排序由时间契约和测试固定。截止后的退出或资源撤回按后续状态规则处理，不能反向改写已有结果；已取消的征集保持终态。
...
| D06b 通知范围 | 报名和修改给页面回执；成行、取消、候补转正、申请审批结果和重大变更发送邮件；发起人可从当前参与名单选择部分或全部成员发送系统邮件，每场最多 5 次并记录审计；普通人数变化仅更新页面 | 已确认，2026-09-17 |
| D06c 公开范围 | 活动介绍、规则、人数及资源进度公开；昵称由成员选择是否公开；详细地址由发布者选择公开或仅已报名成员可见；参与者邮箱仅本场发起人可见，其他管理者、参与者和访客不可见 | 已确认，2026-09-17 |
...
| D11 判定时间边界 | 单一或已确认时段按该时段期限判断；未确认的多个候选日期按相同提前量逐期截止，前序日期过期不取消整场；执行延迟不纳入期限后才生效的变更 | 时间原则已确认；逐期候选规则于 2026-09-11 修正 |
...
### 城市与候选时间（2026-09-07）
面向发布者和报名者：每场活动固定一个城市，不同城市分别发布活动；报名不再收集自由文本地点。发布者以日历多选候选日期，可批量设置并逐日调整开始结束时间。报名者多选可参加时段，保留交通多选及报名留言。每个候选日期按相同提前量依次到达确认期限；到期未确认只淘汰该日期，剩余日期继续征集。发布者确认最终时段后，最低成行人数仅计算该时段中获得席位的人；最后日期仍未确认不能成行。旧固定时间活动继续兼容。
