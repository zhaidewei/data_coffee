...
- 草稿阶段自由修改；发布后锁定最低人数和其他成团规则。发布者可在报名截止前把人数上限提高到 100；不能下调，扩容会通知成员并按原候补顺序递补。其他重大事项变更时关闭旧征集、重新发布，并通知成员重新确认；描述文字可修正并保留修改记录（D02b）。
...
发布后的窄例外是扩充人数上限：仅本场发布者可在报名截止前把上限提高到不超过 100 的整数。有候补且递补截止已过时拒绝；扩至 9 人及以上会强制要求场地；已成行或补齐中的活动在场地规则生效时，现有已批准场地容量必须覆盖新上限。成功后记录扩容前后值、通知成员，并沿用原候补 FIFO 与候补确认机制。
...
- AC17：只有发布者能在报名截止前提高人数上限，输入必须是大于当前上限且不超过 100 的整数；有候补且递补截止已过时拒绝。扩至 9 人及以上强制要求场地；已成行或补齐中若场地规则生效，已批准场地容量必须覆盖新上限。成功产生含扩容前后值的回执和 `capacity_expanded` 通知，并按原 FIFO 顺序递补候补。
...
| D02b 发布后变更 | 草稿自由修改；发布后仅允许发布者在报名截止前上调人数上限至 100，并通知成员、按原候补顺序递补；最低人数和其他规则锁定；其他重大变更关闭旧征集并重新发布；描述修正保留记录 | 已确认，2026-09-09 |
