...
发布后的窄例外是扩充人数上限：仅本场发布者可在报名截止前把上限提高到不超过 100 的整数。有候补且递补截止已过时拒绝；征集中扩至 9 人及以上会强制要求场地；已成行或补齐中的活动仅在现有已批准场地容量覆盖新上限时允许。成功后记录扩容前后值、通知成员，并沿用原候补 FIFO 与候补确认机制。
...
- AC17：只有发布者能在报名截止前提高人数上限，输入必须是大于当前上限且不超过 100 的整数；有候补且递补截止已过时拒绝。征集中扩至 9 人及以上强制要求场地；已成行或补齐中仅在已批准场地容量覆盖新上限时成功。成功产生含扩容前后值的回执和 `capacity_expanded` 通知，并按原 FIFO 顺序递补候补。
