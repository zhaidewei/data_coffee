...
## 2026-09-07 MVP 发布前复核

读者：发布维护者。当前新增时间校验与3–100人规则已纳入复核。
- 独立 peer review：修复批量辅助时间阻止合法跨日时段提交；提交校验只读取实际候选时段。认证、HTTP、CLI 定向测试55项通过。
- 全量规则与存储测试111项通过，包含最低3人、最多100人、主持人0–10的边界。
- 保留根旧系统与设计mock：未证明可删除且不参与 app/web 发布资产；本轮不扩大删除范围。
- CSS多轮覆盖存在维护成本，暂保留有行为作用的规则，后续可按主题变量合并；不以仓促重写作为上线门槛。
- 发布阻断：尚无 data-coffee-dev Worker 和 D1；邮件服务凭证及发件地址待确认。Cloudflare身份可用，D1 ID仍是占位符。当前不可宣布朋友可登录测试。
